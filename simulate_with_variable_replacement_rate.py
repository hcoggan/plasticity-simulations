...
def homeostat_growth(cell_types, cell_driv_muts, cell_epig_alts, deme_pops, num_types, xs, ys, zs, positions, n_demes, neighbour_matrix, order, arrlength, subclone_parentage, muts_per_clone, drivs_per_clone, spno, mno, dno, expno, s, herit, epig_rate, division_prob, driv_prob, deme_size, exp_muts, scratch, ur_nos, ur_tracker, mut_rand_nos, mut_tracker, scratch_limit, no_demes_total=10000, nr=1000000, rep_rate=1):
    ...
    movement_prob = rep_rate*(1-deme_fits/deme_fits[:, np.newaxis])*neighbour_matrix #prob is 0 if they're not neighbours
...
def grow_tumour(expno, scratch, division_prob, driv_prob, exp_muts, gridlength, deme_size, s, herit, epig_rate, no_demes_total, no_cells_total, arrlength, scratch_limit, nr=10000000, total_time=365, rep_rate=1):
...
def run(savepath, expname, expno, scratch, s, herit, no_cells_total, total_time, scratch_limit, division_prob=1-np.exp(-1), driv_prob=0.00001, exp_muts=0.6, gridlength=100, deme_size=10000, arrlength=1000000, nr=1000000, no_samples=8, read_depth=160, detect_thresh=0.01, min_reads=4, error=0.0001, cells_per_sample=50000, gen_drivers=1, nongen_drivers=1, rep_rate=1):
    ...
    start = time.time()
    t, cell_types, deme_pops, num_types, positions, subclone_parentage, muts_per_clone, drivs_per_clone, spno, mno, dno, surf_xs, surf_ys, surf_zs = grow_tumour(expno, scratch, division_prob, adjusted_driv_prob, exp_muts, gridlength, deme_size, s, herit, epig_rate, no_demes_total, no_cells_total,  arrlength, scratch_limit, nr=nr, total_time=total_time, rep_rate=rep_rate)
    end = time.time()
...
def main():
    ...
    parser.add_argument('--rep_rate', type=float, required=True, help='Ease with which cells can migrate')
    
    args = parser.parse_args()
    try:
      np.load(args.savepath+"/"+args.name+"_"+str(args.num)+"_mutdict.npy", allow_pickle=True) #to avoid repeating runs
    except:
      run(args.savepath, args.name, args.num, args.scratch, args.sel, args.herit, int(args.no_cells_total), int(args.total_time), args.scratch_limit, gen_drivers=args.gen_drivers, nongen_drivers=args.nongen_drivers, exp_muts=args.exp_muts, rep_rate=args.rep_rate)
