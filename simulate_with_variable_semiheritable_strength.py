...
def cull_cells_in_one_deme(x, y, z, cell_types, cell_driv_muts, cell_epig_alts, positions, deme_pops, deme_size, s, nongen_s):
    ...
    if pop > deme_size:
        ...
        survival_fitnesses = np.power(1+s, driv_muts_here)*np.power(1+nongen_s, epig_alts_here)
...
def single_deme_division_np(x, y, z, cell_types, cell_driv_muts, cell_epig_alts, deme_pops, num_types, positions, exp_muts, deme_size, s, nongen_s, herit, epig_rate, division_prob, driv_prob):
    pos, pop = positions[x][y][z], deme_pops[x][y][z]
    #find properties
    types_here, driv_muts_here, epig_alts_here = cell_types[pos][:pop], cell_driv_muts[pos][:pop], cell_epig_alts[pos][:pop]
    ...
    division_probs_here = division_prob + (1-division_prob)*(1-np.power((1-s), driv_muts_here)*np.power(1-nongen_s, epig_alts_here)) #allow nongenetic alterations to have separate frequency
...
@njit(parallel=True)
def get_average_deme_fitnesses(fits, epig_alts, pop, s, nongen_s):
    return np.average(np.power((1+s), fits[:pop])*np.power(1+nongen_s, epig_alts[:pop]))


#update this to cull cells in demes one by one
def all_deme_division(xs, ys, zs, cell_types, cell_driv_muts, cell_epig_alts, deme_pops, subclone_parentage, muts_per_clone, drivs_per_clone, num_types, positions, ur_nos, ur_tracker, deme_size, n_demes, nr, exp_muts, s, nongen_s, herit, epig_rate, division_prob, driv_prob, spno, mno, dno, expno, scratch, arrlength, mut_rand_nos, mut_tracker, scratch_limit):
    ...
    for deme in range(n_demes):
        ...
        cell_types, cell_driv_muts, cell_epig_alts, deme_pops, num_types, num_new_types, new_subclone_parentages, new_mut_nos, new_driv_nos =  single_deme_division_np(x, y, z, cell_types, cell_driv_muts, cell_epig_alts, deme_pops, num_types, positions, exp_muts, deme_size, s, nongen_s, herit, epig_rate, division_prob, driv_prob)
        ...
        cell_types, cell_driv_muts, cell_epig_alts, deme_pops = cull_cells_in_one_deme(x, y, z, cell_types, cell_driv_muts, cell_epig_alts, positions, deme_pops, deme_size, s, nongen_s)
        #now update average fitness of this deme
        pop = deme_pops[x][y][z]
        deme_fits[pos_here] = get_average_deme_fitnesses(cell_driv_muts[pos_here], cell_epig_alts[pos_here], pop, s, nongen_s)
...
def initial_growth(cell_types, cell_driv_muts, cell_epig_alts, deme_pops, subclone_parentage, muts_per_clone, drivs_per_clone, num_types, positions, necrotic_marker, ur_nos, ur_tracker, s, nongen_s, herit, epig_rate, division_prob, driv_prob, exp_muts, arrlength, spno, mno, dno, expno, deme_size, scratch, mut_rand_nos, mut_tracker, xs, ys, zs, scratch_limit, no_demes_total=10000, nr=1000000000):
...
def run_to_full_size(expno, scratch, s, nongen_s, herit, epig_rate, division_prob, driv_prob, exp_muts, no_demes_total, no_cells_total, gridlength, deme_size, scratch_limit, nr=100000000, arrlength=1000000):
    num_types = 1
    c = int(gridlength/2)
...
def run_surface_growth(t, cell_types, cell_driv_muts, cell_epig_alts, deme_pops, num_types, positions, necrotic_marker, subclone_parentage, muts_per_clone, drivs_per_clone, spno, mno, dno, expno, nr, arrlength, deme_size, scratch, exp_muts, s, nongen_s, herit, epig_rate, division_prob, driv_prob, no_demes_total, total_time, gridlength, xs, ys, zs, scratch_limit):
...
def grow_tumour(expno, scratch, division_prob, driv_prob, exp_muts, gridlength, deme_size, s, nongen_s, herit, epig_rate, no_demes_total, no_cells_total, arrlength, scratch_limit, nr=10000000, total_time=365):
    t, cell_types, cell_driv_muts, cell_epig_alts, deme_pops, num_types, positions, necrotic_marker, subclone_parentage, muts_per_clone, drivs_per_clone, spno, mno, dno, xs, ys, zs  = run_to_full_size(expno, scratch, s, nongen_s, herit, epig_rate, division_prob, driv_prob, exp_muts, no_demes_total, no_cells_total, gridlength, deme_size, scratch_limit, nr=nr, arrlength=arrlength)
...
#now, a function to get the counts of all mutations in a sample without seeing the whole thing
#spno, dno, mno should all be the same at end of sim
def compile_muts_all_samples(scratch, chosen_sample_clone_dict, sample_pops, subclone_parentage, muts_per_clone, drivs_per_clone, arrlength, arrno, expno, detect_thresh=4, no_samples=8, reads=160, rel_thresh=0.01, error=0.0001):
    #use a dictionary of clones whose parentage we're looking to trace in each sample, and their counts
    events_to_trace, events_in_sample = chosen_sample_clone_dict.copy(), [{} for n in range(no_samples)]
    muts_per_event, drivs_per_event = {}, {} #keep track of
    #how many mutations we have at each point
    min_mut = arrno*arrlength
    current_array = arrno
    all_drivers = []
    while current_array >= 0: #we will definitely need to get all the way to the end
        new_events_to_trace = []
        for n, (eventdict, sampleeventdict) in enumerate(zip(events_to_trace, events_in_sample)):
            new_events_to_trace_here = {}
            for event, count in eventdict.items(): #for each clone
                anc = event
                while anc >= min_mut and anc > 1: #while the thing we're looking for is in range, and not the root
                    #it is added to all lists if and when we find it in the current array, which MEANS that all of its descendants must already have been found
                    if anc in sampleeventdict: #it has been found before
                        sampleeventdict[anc] += count
                    else:
                        sampleeventdict[anc] = count
                    #print(anc, min_mut)
                    if anc not in muts_per_event:
                        muts_per_event[anc] = muts_per_clone[anc - min_mut] 
                        drivs_per_event[anc] = drivs_per_clone[anc - min_mut] #doesn't matter if these are already here, they'll never change
                    anc = subclone_parentage[anc- min_mut] #find parent
                else: #it's not in current list
                    if anc > 1: #not the root
                        if anc in new_events_to_trace_here:
                            new_events_to_trace_here[anc] += count #this is the ancestor of multiple clones in this array
                        else:
                            new_events_to_trace_here[anc] = count #multiple descendants may converge to the same out of range ancestor
            new_events_to_trace.append(new_events_to_trace_here) #now each sample has a list of unfound events and the counts that are attached to them
        #now update everything
        current_array -= 1
        if current_array >= 0:
            subclone_parentage, muts_per_clone, drivs_per_clone = np.load(scratch+"/"+str(expno)+"_subclone_parentage_"+str(current_array)+".npy", allow_pickle=True), np.load(scratch+"/"+str(expno)+"_muts_per_clone_"+str(current_array)+".npy", allow_pickle=True), np.load(scratch+"/"+str(expno)+"_drivs_per_clone_"+str(current_array)+".npy", allow_pickle=True)
            min_mut = current_array*arrlength
        events_to_trace = new_events_to_trace
    #at the end this should be an empty array, all of the roots being 1- this does work
    print("mutations discovered at each point")
    #now assign unique indices to each mutation
    all_muts = np.array(list(muts_per_event.values()))
    first_indices = np.subtract(np.cumsum(all_muts), all_muts)
    event_index_dict = dict(zip(list(muts_per_event.keys()), list(first_indices)))
    #now we have dictionaries listing the first indices, number of mutations and number of drivers associated with each event
    all_drivers = []
    for event in muts_per_event.keys():
        if drivs_per_event[event] > 0:
            all_drivers += list(range(event_index_dict[event], event_index_dict[event]+drivs_per_event[event])) #add these to the list of drivers
    #and now sequence everything at once!
    all_vafs_in_sample = [{} for n in range(no_samples)]
    for n, (sampleeventdict, pop) in enumerate(zip(events_in_sample, sample_pops)):
        all_muts_here, all_prevs_here = [], []
        #now we have a list of all the prevalences
        #compile this into mutations
        for event, count in sampleeventdict.items():
            all_muts_here += list(range(event_index_dict[event], event_index_dict[event]+muts_per_event[event]))
            all_prevs_here += [count]*muts_per_event[event]
        all_muts_here, prevs = np.array(all_muts_here), np.array(all_prevs_here)/pop
        #filter out those under 1%
        detectable = np.where(prevs >= rel_thresh)[0]
        all_muts_here, prevs = all_muts_here[detectable], prevs[detectable]
        #add in all witnessed mutations
        rs = np.random.binomial(reads, prevs*(1-error))
        #filter those with reads < 4
        detectable = np.where(rs >= detect_thresh)[0]
        all_muts_here, vafs = all_muts_here[detectable], rs[detectable]/reads
        all_vafs_in_sample[n] = dict(zip(list(all_muts_here), list(vafs)))
    return all_vafs_in_sample, all_drivers
...
def run(savepath, expname, expno, scratch, s, herit, no_cells_total, total_time, scratch_limit, division_prob=1-np.exp(-1), driv_prob=0.00001, exp_muts=0.6, gridlength=100, deme_size=10000, arrlength=1000000, nr=1000000, no_samples=8, cells_per_sample=50000, min_reads=4, read_depth=160, detect_thresh=0.01, error=0.0001, gen_drivers=1, nongen_drivers=1, q=1, f=1):
    no_demes_total = int(no_cells_total/deme_size) #by assumption
    epig_rate = f*driv_prob*exp_muts if nongen_drivers==1 else 0 #by assumption- same as probability of drivers, if using, scaled by relative frequency
    ...
    nongen_s = q*s #strength of nongenetic drivers
    start = time.time()
    t, cell_types, deme_pops, num_types, positions, subclone_parentage, muts_per_clone, drivs_per_clone, spno, mno, dno, surf_xs, surf_ys, surf_zs = grow_tumour(expno, scratch, division_prob, adjusted_driv_prob, exp_muts, gridlength, deme_size, s, nongen_s, herit, epig_rate, no_demes_total, no_cells_total,  arrlength, scratch_limit, nr=nr, total_time=total_time)
    end = time.time()
...
def main():
    ...
    parser.add_argument('--q', type=float, required=True, help='Relative strength of nongenetic to genetic drivers')
    parser.add_argument('--f', type=float, required=True, help='Relative frequency of nongenetic to genetic drivers')
    parser.add_argument('--batch_size', type=int, required=True, help='Number of jobs in a batch')
    args = parser.parse_args()
    scratch = args.scratch + "/"+str(args.num)
    for b in range(args.batch_size):
      exp_num = args.batch_size*args.num + b + 1 #everything is saved into the same scratch space and then loaded back into memory in order-- everything necessary has been overwritten
      try:
        np.load(args.savepath+"/"+args.name+"_"+str(exp_num)+"_mutdict.npy", allow_pickle=True) #to avoid repeating runs
      except:
        try:
          run(args.savepath, args.name, exp_num, scratch, args.sel, args.herit, int(args.no_cells_total), int(args.total_time), args.scratch_limit, gen_drivers=args.gen_drivers, nongen_drivers=args.nongen_drivers, exp_muts=args.exp_muts, q=args.q, f=args.f)
        except:
          print(exp_num, "failed")
