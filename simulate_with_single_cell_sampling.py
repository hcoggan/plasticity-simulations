...
def grow_tumour(expno, scratch, division_prob, driv_prob, exp_muts, gridlength, deme_size, s, herit, epig_rate, no_demes_total, no_cells_total, arrlength, scratch_limit, nr=10000000, total_time=365):
    ...
    return t, cell_types, cell_epig_alts, deme_pops, num_types, positions, subclone_parentage, muts_per_clone, drivs_per_clone, spno, mno, dno, surf_xs, surf_ys, surf_zs
...
#now choose a random deme and get its cells and their fitnesses, for a given number of cells
def extract_cells_and_fitnesses(cell_types, cell_epig_alts, deme_pops, surf_xs, surf_ys, surf_zs, positions, no_cells_to_take):
    #make sure we choose a deme with enough cells in it- if we give a value greater than 10k or so we're going to cause problems for ourselves
    sampled_enough_cells = False
    while not sampled_enough_cells:
        deme_chosen = np.random.randint(len(surf_xs)) #choose a deme at random
        #len(surf_xs) should also be the length of cell_types at this point
        x, y, z = surf_xs[deme_chosen], surf_ys[deme_chosen], surf_zs[deme_chosen]
        pos, pop = positions[x][y][z], deme_pops[x][y][z]
        sampled_enough_cells = (pop >= no_cells_to_take)
    #now we have found a sufficiently populous region
    types, epig_alts = cell_types[pos][:pop], cell_epig_alts[pos][:pop]
    #now choose a number of cells at random
    indices_to_sample = np.random.choice(pop, size=no_cells_to_take, replace=False)
    sampled_types, sampled_alts = types[indices_to_sample], epig_alts[indices_to_sample]
    return sampled_types, sampled_alts
...
#get a list of mutations per type, and the distance between each type
def get_distance_matrix(all_clones_seen, types_to_index_dict, muts_per_type, parentage_dict):
    list_of_mutations_per_type = []
    for type in all_clones_seen:
        anc = type
        mutations = []
        while anc != 1:
            position = types_to_index_dict[anc] #find instantiating muts
            [mut_at_start, mut_at_end] = muts_per_type[position] #find instantiating muts
            mutations += list(range(mut_at_start, mut_at_end)) #add to the list
            anc = parentage_dict[anc] #work your way up the list
        list_of_mutations_per_type.append(mutations) #add to the list
    #now we have a list of mutations in the same order as all_clones_seen
    distance_matrix_num = np.zeros((len(all_clones_seen), len(all_clones_seen)), dtype=np.int64)
    distance_matrix_frac = np.zeros((len(all_clones_seen), len(all_clones_seen)))
    for n in range(len(all_clones_seen)):
        ls_n = np.array(list_of_mutations_per_type[n])
        for m in range(n):
            ls_m = np.array(list_of_mutations_per_type[m])
            all_muts_between = len(np.unique(np.concatenate((ls_n, ls_m)))) #how many appear in either
            all_muts_different = len(np.setdiff1d(ls_n, ls_m)) + len(np.setdiff1d(ls_m, ls_n))  #how many only appear in 1
            distance_matrix_num[m][n], distance_matrix_num[n][m] = all_muts_different, all_muts_different
            distance_matrix_frac[m][n], distance_matrix_frac[n][m] = all_muts_different/all_muts_between, all_muts_different/all_muts_between
    return distance_matrix_num, distance_matrix_frac
            
            
        


#put all of this together to take single cell
def single_cell_sequencing(scratch, cell_types, cell_epig_alts, deme_pops, surf_xs, surf_ys, surf_zs, positions, no_cells_to_take,  subclone_parentage, muts_per_clone, drivs_per_clone, arrlength, arrno, expno):
    sampled_types, sampled_alts =  extract_cells_and_fitnesses(cell_types, cell_epig_alts, deme_pops, surf_xs, surf_ys, surf_zs, positions, no_cells_to_take)
    all_clones_seen = np.unique(sampled_types) #types to sequence- this is in order
    parentage_dict, muts_per_type_dict = construct_tree(scratch, all_clones_seen.copy(), subclone_parentage.copy(), muts_per_clone.copy(), drivs_per_clone.copy(), arrlength, arrno, expno)
    types_to_index_dict, muts_per_type =  get_mutations_per_clone(parentage_dict, muts_per_type_dict)
    distance_matrix_num, distance_matrix_frac =  get_distance_matrix(all_clones_seen, types_to_index_dict, muts_per_type, parentage_dict)
    return sampled_types, sampled_alts, all_clones_seen, distance_matrix_num, distance_matrix_frac
...
def run(savepath, expname, expno, scratch, s, herit, no_cells_total, total_time, scratch_limit, division_prob=1-np.exp(-1), driv_prob=0.00001, exp_muts=0.6, gridlength=100, deme_size=10000, arrlength=1000000, nr=1000000, no_samples=8, read_depth=160, detect_thresh=0.01, min_reads=4, error=0.0001, cells_per_sample=50000, gen_drivers=1, nongen_drivers=1):
    ...
    start = time.time()
    t, cell_types, cell_epig_alts, deme_pops, num_types, positions, subclone_parentage, muts_per_clone, drivs_per_clone, spno, mno, dno, surf_xs, surf_ys, surf_zs = grow_tumour(expno, scratch, division_prob, adjusted_driv_prob, exp_muts, gridlength, deme_size, s, herit, epig_rate, no_demes_total, no_cells_total,  arrlength, scratch_limit, nr=nr, total_time=total_time)
    end = time.time()
    print(end-start, "seconds to simulate tumour")
    for no_cells_to_take in [100, 200, 500, 1000]:
        sampled_types, sampled_alts, all_clones_seen, distance_matrix_num, distance_matrix_frac = single_cell_sequencing(scratch, cell_types, cell_epig_alts, deme_pops, surf_xs, surf_ys, surf_zs, positions, no_cells_to_take,  subclone_parentage, muts_per_clone, drivs_per_clone, arrlength, spno, expno)
        #now save what you need-- these are raw prevalences
        np.save(savepath+"/"+expname+"_"+str(expno)+"_sampled_types_"+str(no_cells_to_take)+".npy", sampled_types)
        np.save(savepath+"/"+expname+"_"+str(expno)+"_sampled_alts_"+str(no_cells_to_take)+".npy", sampled_alts)
        np.save(savepath+"/"+expname+"_"+str(expno)+"_unique_types_"+str(no_cells_to_take)+".npy", all_clones_seen)
        np.save(savepath+"/"+expname+"_"+str(expno)+"_abs_dist_"+str(no_cells_to_take)+".npy", distance_matrix_num)
        np.save(savepath+"/"+expname+"_"+str(expno)+"_frac_dist_"+str(no_cells_to_take)+".npy", distance_matrix_frac)
...
def main():
    ...
    for b in range(args.batch_size):
      ...
      try:
        ...
        try:
          run(args.savepath, args.name, exp_num, scratch, args.sel, args.herit, int(args.no_cells_total), int(args.total_time), args.scratch_limit, gen_drivers=args.gen_drivers, nongen_drivers=args.nongen_drivers, exp_muts=args.exp_muts)
        except:
